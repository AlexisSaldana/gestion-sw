        ...
        <div class="mx-auto w-full max-w-[550px]">
            <form action="{{ route('consultas.update', ['id' => $consulta->id]) }}" method="POST">
                ...
                <div class="mb-5">
                    ...
                    <textarea id="diagnostico" name="diagnostico" placeholder="Ingrese su diagnóstico aquí" required class="w-full resize-none rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md">{{ $consulta->diagnostico }}</textarea>
                </div>
                <div class="mb-5">
                    <label for="receta" class="mb-3 block text-base font-medium text-[#07074D]">
                        Receta
                    </label>
                    <textarea id="receta" name="receta" placeholder="Detalle la receta aquí" class="w-full resize-none rounded-md border border-[#e0e0e0] bg-white py-3 px-6 text-base font-medium text-[#6B7280] outline-none focus:border-[#6A64F1] focus:shadow-md">{{ $consulta->receta }}</textarea>
                </div>
                <div class="mb-5">
                    <label class="mb-3 block text-base font-medium text-[#07074D]">Productos</label>
                    @foreach($productos as $producto)
                    <div class="form-check mb-2">
                        <input type="checkbox" class="form-check-input product-checkbox" id="producto_{{ $producto->id }}" name="productos[]" value="{{ $producto->id }}" data-price="{{ $producto->precio }}" {{ $consulta->productos->contains($producto->id) ? 'checked' : '' }}>
                ...
                <div class="mb-5">
                    ...
                    <div class="form-check mb-2">
                        <input type="checkbox" class="form-check-input service-checkbox" id="servicio_{{ $servicio->id }}" name="servicios[]" value="{{ $servicio->id }}" data-price="{{ $servicio->precio }}" {{ $consulta->servicios->contains($servicio->id) ? 'checked' : '' }}>
