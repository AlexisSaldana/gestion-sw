                ...
                <div class="p-6 text-gray-900">
                    <div class="overflow-x-auto bg-white dark:bg-neutral-700">
                        ...
                        <form method="GET" action="{{ route('servicios') }}" class="flex my-4 mx-4 items-center">
                            ...
                            <button type="reset" onclick="window.location='{{ route('servicios') }}'" class="ml-4 px-4 py-2 bg-gray-500 text-white font-semibold rounded-md hover:bg-red-600">
                                Reiniciar
                            </button>
                        </form>
                                ...
                                @foreach($servicios as $servicio)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4">
                                            ...
                                            <button class="openEditModalButton text-blue-500 hover:text-blue-700" data-id="{{ $servicio->id }}">
                                                Editar
                                            </button>
    ...
    <script>
        ...
        document.querySelectorAll('.openEditModalButton').forEach(button => {
            button.addEventListener('click', function() {
                ...
                fetch(`/secretaria/servicios/editar/${servicioId}`)
                    .then(response => response.json())
                    .then(data => {
                        document.getElementById('edit_nombre').value = data.nombre;
                        document.getElementById('edit_precio').value = data.precio;
                        document.getElementById('editForm').action = `/secretaria/servicios/editar/${servicioId}`;
